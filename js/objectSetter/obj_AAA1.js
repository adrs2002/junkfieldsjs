...
function Obj_AAA_1() {
    ObjectSetter.call(this, "AAA");
}

//オブジェクトのロードメソッド
Obj_AAA_1.prototype.load = function (object) {

    var self = this;

    self.modelObject = object.FrameInfo[0]; //XfileLoader_CopyObject(object.FrameInfo[0]);
    self.modelObject.scale.setScalar(Master_Chara_Size);
    ...
    self.meca_Stat_Const_f[cEnConsts_ParamF.warkS_up] = 0.005;//歩行速度上昇値	
    self.meca_Stat_Const_f[cEnConsts_ParamF.warkS_max] = 0.25; //歩行速度最大値	
    ...
    self.meca_Stat_Const_f[cEnConsts_ParamF.body_height] = 3.5 * Master_Chara_Size; //地上からの機体の中心・高さ	
    ...
    self.meca_Stat_Const_f[cEnConsts_ParamF.life_max] = 555; //HP最大値	
    ...
    self.UpBodyBone = self.modelObject.skeleton.bones[2];   //X軸も向かせたいなら  3 
    if (self.UpBodyBone.appendMatrix == undefined) {
        self.UpBodyBone.appendMatrix = new THREE.Matrix4();
    }
    self.UpBodyBone.BaseMatrix = new THREE.Matrix4();
    self.UpBodyBone.BaseMatrix.copy(self.UpBodyBone.matrix);
    self.UpBodyBone.BaseMatrix.elements[5] = 1.0;
    self.UpBodyBone.BaseMatrix.elements[10] = 1.0;
    self.UpBodyBone.BaseMatrix.elements[13] *= -1.0;
    self.UpBodyBone.BaseMatrix.elements[14] *= -1.0;

    self.mountWpnBone[0] = self.modelObject.skeleton.bones[3];
...
inherits(Obj_AAA_1, ObjectSetter);
